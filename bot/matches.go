...
func (b *Bot) commandScheduleMatch(ctx context.Context, data cmdroute.CommandData) (resp *api.InteractionResponseData) {
	...
	var (
		...
		team1, team2        discord.RoleID
		moderatorID         discord.UserID
		streamerID          discord.UserID
		okStreamer          bool
		streamUrl           string
	)
	...
	// start async, because longer running
	go func(
		scheduledAt time.Time,
		participantsPerTeam int64,
		team1, team2 discord.RoleID,
	...
	return &api.InteractionResponseData{
		Content: option.NewNullableString(
			"Accepted, a new match channel will be created shortly.",
		),
